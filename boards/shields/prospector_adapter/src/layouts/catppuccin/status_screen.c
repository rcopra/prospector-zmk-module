...
static struct catppuccin_state state;
...
static void update_display(struct k_work *work) {
    ...
    struct catppuccin_state snapshot = state;
    k_mutex_unlock(&state_mutex);
    catppuccin_update(&snapshot);
}
...
static int status_listener(const zmk_event_t *event) {
    ...
    if (key && key->state) {
        state.right_paw = !state.right_paw;
    }
...
ZMK_LISTENER(catppuccin_status, status_listener);
ZMK_SUBSCRIPTION(catppuccin_status, zmk_peripheral_battery_state_changed);
ZMK_SUBSCRIPTION(catppuccin_status, zmk_split_central_status_changed);
ZMK_SUBSCRIPTION(catppuccin_status, zmk_keycode_state_changed);
ZMK_SUBSCRIPTION(catppuccin_status, zmk_layer_state_changed);
ZMK_SUBSCRIPTION(catppuccin_status, zmk_endpoint_changed);
ZMK_SUBSCRIPTION(catppuccin_status, zmk_ble_active_profile_changed);
ZMK_SUBSCRIPTION(catppuccin_status, zmk_usb_conn_state_changed);
ZMK_SUBSCRIPTION(catppuccin_status, zmk_wpm_state_changed);
#ifdef CONFIG_DT_HAS_ZMK_BEHAVIOR_CAPS_WORD_ENABLED
ZMK_SUBSCRIPTION(catppuccin_status, zmk_caps_word_state_changed);
#endif

lv_obj_t *zmk_display_status_screen(void) {
    lv_obj_t *screen = catppuccin_create();
    k_mutex_lock(&state_mutex, K_FOREVER);
    refresh_keyboard_state();
    k_mutex_unlock(&state_mutex);
    update_display(NULL);
